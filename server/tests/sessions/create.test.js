...
const { createSessionController } = require("../../src/controllers/sessions");
...
describe("POST /api/sessions", () => {
  ...
  it("C2: creates distinct sessions for identical requests", async () => {
    ...
    const first = await request(app).post("/api/sessions").send(input);

    const second = await request(app).post("/api/sessions").send(input);

    expect(first.status).toBe(201);
    expect(second.status).toBe(201);

    expect(first.body.id).not.toBe(second.body.id);
    expect(first.body.instructorToken).not.toBe(second.body.instructorToken);
  });
  ...
  it("creates default playback state for the session", async () => {
    const response = await request(app).post("/api/sessions").send({
      name: "Q3 Onboarding Training",
      youtubeUrl: "https://www.youtube.com/watch?v=dQw4w9WgXcQ",
    });
